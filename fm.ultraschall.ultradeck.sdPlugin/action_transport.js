const transport = {
  ...
  onWillAppear: function (jsn) {
    ...
    $SD.api.setImage(jsn.context, 'data:image/svg+xml;base64,' + btoa(Icons[this.settings.icon_playstate[0]]));
          ...
          if (xhttp.status === 200) {
            ...
            if (playstate_last !== playstate || isRepeatOn_last !== isRepeatOn) {
              if (isRepeatOn === "1" && Icons[settings.icon_playstate["R"]] !== undefined) {
                $SD.api.setImage(jsn.context, 'data:image/svg+xml;base64,' + btoa(Icons[settings.icon_playstate["R"]]));
              }
              else {
                $SD.api.setImage(jsn.context, 'data:image/svg+xml;base64,' + btoa(Icons[settings.icon_playstate[playstate]]));
              }
            }
